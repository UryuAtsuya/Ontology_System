"""
gemini_fewshot_eval.py (fixed)
---------------------------------
概要:
  Gemini 応答を few‑shot / zero‑shot で収集し、定量 (複数メトリクス)・定性の両面で比較評価する。
  - 依存ライブラリが無い環境でも動作するようフォールバックを実装。
  - BERTScore は導入済みなら自動で使用。未導入なら ROUGE‑L / token‑F1 等を代替指標として出力。

更新日: 2025/09/22
    
更新点:
  - rapidfuzz / bert_score / tqdm が無い場合も落ちないようにした。
  - Levenshtein 距離は純 Python 実装にフォールバック。
...
def call_gemini(prompt: str, system_instruction: str | None = None, model_name: str = "gemini-1.5-flash", max_retries: int = 3) -> str:
...
def main():
    ...
    p.add_argument("--model", type=str, default="gemini-1.5-flash")
    p.add_argument("--runs", type=int, default=5)
    p.add_argument("--output", type=str, default="gemini_results", help="出力ファイル接頭辞")
    args = p.parse_args()
    ...
    # reference: few‑shot 最初の応答
    reference = fewshot_answers[0] if fewshot_answers else ""

    # ---------------- 定量: 複数メトリクス ----------------
    print("\\n[3] 定量評価 (reference = few‑shot[0])")
